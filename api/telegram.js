...
// Новая функция для формирования подробной рекомендации с эмодзи и развернутым текстом
function generateDetailedRecommendation(price, sma5, rsiVal, candlePattern) {
  ...
  // Эмодзи для настроения рекомендации
  let emoji = '❓';
  let recommendation = '';
  ...
  if (priceAboveSMA && !rsiOverbought && candlePattern && candlePattern.includes('Молот')) {
    emoji = '📈🛠️';
    recommendation =
      `Предположим, цена находится выше 50-дневной скользящей средней (${sma5.toFixed(5)}), ` +
      `что указывает на восходящий тренд. Индикатор RSI равен ${rsiVal.toFixed(1)}, ` +
      `что не является экстремальным значением и говорит об отсутствии перекупленности. ` +
      `На графике обнаружен свечной паттерн "молот" — бычий сигнал разворота.\n\n` +
      `Это может указывать на то, что цена, вероятно, продолжит расти, так как рынок не перекуплен, ` +
      `скользящая средняя поддерживает восходящий тренд, и есть бычий свечной паттерн.`;
  } else if (!priceAboveSMA && rsiOverbought && candlePattern && candlePattern.includes('Повешенный')) {
    emoji = '📉⚠️';
    recommendation =
      `Если цена находится ниже 50-дневной скользящей средней (${sma5?.toFixed(5) || 'N/A'}), ` +
      `а RSI показывает значение ${rsiVal.toFixed(1)}, указывающее на перекупленность рынка, ` +
      `и на графике виден паттерн "повешенный" — медвежий сигнал разворота.\n\n` +
      `Это может указывать на вероятное снижение цены в ближайшей перспективе.`;
  } else if (priceAboveSMA && rsiOversold) {
    emoji = '🔄📊';
    recommendation =
      `Цена находится выше скользящей средней (${sma5.toFixed(5)}), но RSI равен ${rsiVal.toFixed(1)}, ` +
      `что указывает на перепроданность рынка.\n\n` +
      `Возможен разворот вверх, так как цена поддерживается трендом, и рынок может начать восстанавливаться.`;
  } else if (!priceAboveSMA && rsiOverbought) {
    emoji = '⚠️📉';
    recommendation =
      `Цена ниже скользящей средней (${sma5?.toFixed(5) || 'N/A'}), а RSI высокий (${rsiVal.toFixed(1)}), ` +
      `что говорит о перекупленности рынка.\n\n` +
      `Возможен разворот вниз или коррекция, рекомендуется быть осторожным с покупками.`;
  } else {
    emoji = '❗️';
    recommendation =
      `Ситуация неоднозначна, индикаторы не дают чётких сигналов.\n` +
      `Рекомендуется дождаться подтверждающих сигналов и провести дополнительный анализ перед принятием решения.`;
  }

  // Добавим в начало эмодзи для наглядности
...
function analyzeIndicators(klines, sma5, sma15, rsi, macd, stochastic, supports, resistances) {
  ...
  // Итоговые выводы с подробной рекомендацией
  text += '\n📌 Рекомендация:\n';
