...
// Настройки WebSocket для Pocket Option API
const POCKET_OPTION_WS_URL = 'wss://api.po.market/socket.io/?EIO=3&transport=websocket';
let ws;
...
const authData = JSON.stringify([
  ...
  [
    "auth",
    {
      "sessionToken": "66d566bf45f739b6cd9462819ae3b475",
      "uid": "102816211",
      "lang": "ru",
      "currentUrl": "cabinet",
      "isChart": 1
...
const languages = {
  ru: {
    ...
    texts: {
      ...
      errorFetchingData: 'Ошибка при получении данных с API.',
  ...
  en: {
    ...
    texts: {
      ...
      errorFetchingData: 'Error fetching data from API.',
...
// Функция подключения к WebSocket API Pocket Option
function connectToPocketOption() {
  ws = new WebSocket(POCKET_OPTION_WS_URL);
  ...
  ws.on('message', (data) => {
    ...
    if (message.startsWith('0')) {
      ...
      console.log('Received data from Pocket Option:', message);
    }
  });

  ws.on('close', () => {
    console.log('Disconnected from Pocket Option WebSocket');
    isWsConnected = false;
...
// Функция для получения данных OHLC через API Pocket Option
async function getOHLCData(pair, timeframeMinutes, count = 100) {
  if (!isWsConnected) {
    throw new Error('WebSocket is not connected');
  }

  return new Promise((resolve, reject) => {
    const timeframeInSeconds = timeframeMinutes * 60;
    const requestData = JSON.stringify([
      42,
      [
        "candles",
        {
          "asset": pair.replace('OTC_', ''), // Убираем префикс OTC для API
          "period": timeframeInSeconds,
          "amount": count,
          "isTurbo": pair.startsWith('OTC_') ? true : false
        }
      ]
    ]);
    ...
    const handler = (data) => {
      const message = data.toString();
      if (message.includes('"candles"')) {
        try {
          const parsedData = JSON.parse(message.slice(2)); // Убираем префикс "42"
          if (parsedData[1] && parsedData[1].candles) {
            const candles = parsedData[1].candles.map(candle => ({
              openTime: candle.time * 1000,
              open: parseFloat(candle.open),
              high: parseFloat(candle.high),
              low: parseFloat(candle.low),
              close: parseFloat(candle.close),
              closeTime: (candle.time + timeframeInSeconds) * 1000 - 1,
    ...
    setTimeout(() => {
      ws.off('message', handler);
      reject(new Error('Timeout waiting for candles data'));
...
bot.on('callback_query', async (ctx) => {
  ...
  if (tf) {
    ...
    let klines;
    try {
      klines = await getOHLCData(ctx.session.pair, tf.minutes, 100);
...
// Запуск бота
connectToPocketOption(); // Подключаемся к WebSocket API Pocket Option
bot.launch();
