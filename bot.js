...
class CurrencyQuotesManager {
  constructor() {
    ...
    this.watchedPairs = new Set(['EURUSD', 'GBPUSD', 'USDJPY', 'EURGBP', 'AUDUSD']);
  }
...
bot.command('quotes', async (ctx) => {
  ...
  quotes.forEach(quote => {
    ...
    message += `Время: ${new Date(quote.timestamp).toLocaleTimeString('ru-RU')}\n\n`;
  });
...
Время: ${new Date(quote.timestamp).toLocaleTimeString('ru-RU')}
      `;
      
      await ctx.reply(message);
    } else {
      await ctx.reply(`Котировка для ${displayNames[text]?.ru || text} недоступна`);
    }
  }
});

bot.launch();
console.log('Currency Quotes Bot started successfully!');
