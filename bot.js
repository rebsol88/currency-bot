...
const worker = Tesseract.createWorker(); // Обратите внимание! Без параметров

// Асинхронная инициализация воркера
(async () => {
  await worker.load();
...
// Простая функция вычисления SMA
...
// Упрощённые OHLC данные с постоянной ценой
...
// Генерация графика с Close и SMA
...
// Парсер пары и цены из распознанного текста OCR
...
bot.on('photo', async ctx => {
  try {
    await ctx.reply('Обрабатываю скриншот, ждите...');
...
bot.start(ctx => ctx.reply('Привет! Отправьте скриншот с графиком валютной пары и ценой.'));
