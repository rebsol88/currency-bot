...
const PO_SESSION_TOKEN = '66d566bf45f739b6cd9462819ae3b475'; // Ваш токен Pocket Option
const PO_UID = '102816211'; // Ваш UID Pocket Option
...
// --- Конфигурация языков, пар, таймфреймов (без изменений) ---
const languages = {
  ru: {
    name: 'Русский',
    pairsMain: [
...
// --- Вспомогательная функция для сопоставления пары с API Pocket Option ---
// Pocket Option API использует имена в формате "EURUSD", "EURUSD-OTC" и т.п.
// Для OTC добавим суффикс "-OTC"
function mapPairToPocketOptionSymbol(pair) {
  if (pair.startsWith('OTC_')) {
    return pair.replace('OTC_', '') + '-OTC';
  }
  return pair;
}

// --- Функция получения реальных OHLC данных через Pocket Option API ---
// https://pocketoption.com/api/v1/market/candles?symbol=EURUSD&interval=1m&count=100
async function fetchRealOHLCFromPocketOption(pair, timeframe, count) {
  const symbol = mapPairToPocketOptionSymbol(pair);
  try {
    const response = await axios.get('https://pocketoption.com/api/v1/market/candles', {
      params: {
        symbol,
        interval: timeframe,
        count,
      ...
      headers: {
        'Authorization': `Bearer ${PO_SESSION_TOKEN}`,
        'Content-Type': 'application/json',
      },
    });

    if (!response.data || !response.data.data) {
      throw new Error('Invalid response from Pocket Option API');
    }

    // Преобразуем данные в формат klines, аналогичный fake
    // API возвращает массив объектов с {time, open, close, high, low, volume}
    // time - unix timestamp в секундах
    const klines = response.data.data.map(candle => ({
      openTime: candle.time * 1000,
      open: candle.open,
      high: candle.high,
      low: candle.low,
      close: candle.close,
      closeTime: candle.time * 1000 + (timeframeToMs(timeframe) - 1),
      volume: candle.volume || 0,
    }));

    return klines;
  } catch (e) {
    console.error('Error fetching data from Pocket Option:', e.message);
    throw e;
  }
}

// --- Перевод таймфрейма в миллисекунды ---
...
// --- Индикаторы и анализ (оставлены без изменений) ---
// ... (весь ваш код с расчетами SMA, RSI, MACD, Stochastic, анализом и генерацией рекомендаций)

// --- Функция генерации графика (оставлена без изменений) ---
// ... (ваш код generateChartImage)
...
bot.on('callback_query', async (ctx) => {
  ...
  if (tf) {
    ...
    try {
      // Запрашиваем реальные данные с Pocket Option
      const klines = await fetchRealOHLCFromPocketOption(ctx.session.pair, tf.value, 100);
